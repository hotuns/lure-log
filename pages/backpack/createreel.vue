...
const { data, pending, refresh } = await useAsyncData(
  "backpack_randomreels",
  () => backPackStore.loadRandomReels()
);
...
<template>
  <div>
    <van-search
      v-model="searchQuery"
      placeholder="搜索卷线器品牌或者型号"
      @search="fetchReels"
    />
    <van-list>
      <van-cell
        v-for="rod in searchResult"
        :key="rod.id"
        :title="rod.model"
        :label="formatLabel(rod)"
        @click="onListClick(rod)"
        value="选择"
        is-link
      />
    </van-list>
    ...
    <van-list>
      <van-cell
        v-for="reel in backPackStore.getRandomReels"
        :key="reel.id"
        :title="reel.model"
        :label="formatLabel(reel)"
        @click="onListClick(reel)"
        value="选择"
        is-link
      />
    </van-list>
